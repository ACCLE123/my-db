...
| 1      | 1            | IS_ROOT                         |
| 2      | 4            | PARENT_POINTER                  |
| 6      | 4            | LEAF_NODE_NUM_CELLS             |
| 10     | 4            | LEAF_NODE_NEXT_LEAF             |
| 14     | (varies)     | LEAF_NODE_CELLS                 |
|        |              |   |- LEAF_NODE_KEY (4 bytes)    |
|        |              |   |- LEAF_NODE_VALUE (ROW_SIZE) |
| ...    | ...          | ...                             |
| 4096   |              | (End of PAGE)                   |
...
| 1      | 1            | IS_ROOT                          |
| 2      | 4            | PARENT_POINTER                   |
| 6      | 4            | INTERNAL_NODE_NUM_KEYS           |
| 10     | 4            | INTERNAL_NODE_RIGHT_CHILD        |
| 14     | (varies)     | INTERNAL_NODE_CELLS              |
|        |              |   |- INTERNAL_NODE_CHILD (4 bytes)|
|        |              |   |- INTERNAL_NODE_KEY (4 bytes)  |
| ...    | ...          | ...                              |
| 4096   |              | (End of PAGE)                    |
...
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
    ...
    while (l != r) {
        uint32_t mid = (l + r) >> 1;
        if (*leaf_node_key(node, mid) >= key) r = mid;
        else l = mid + 1;
    }
...
uint32_t internal_node_find_child(void* node, uint32_t key) {
    ...
    while (l != r) {
        uint32_t mid = (l + r) >> 1;
        if (*internal_node_key(node, mid) >= key) r = mid;
        else l = mid + 1;
    }
    return l;
}
...
Cursor* internal_node_find(Table* table, uint32_t page_num, uint32_t key) {
    void* node = get_page(table->pager, page_num);
    uint32_t pos = internal_node_find_child(node, key);
    uint32_t child_num = *internal_node_child(node, pos);
...
PerpareResult prepare_insert(InputBuffer* input_buffer, Statement* statement) {
...
PerpareResult prepare_statement(InputBuffer* input_buffer, Statement* statement) {
    ...
    *leaf_node_next_leaf(old_node) = new_page_num;
    ...
    for (int32_t i = LEAF_NODE_MAX_CELLS; i >= 0; i--) {
        ...
        if (i == cursor->cell_num) {
            ...
            *leaf_node_key(destination, index_within_node) = key;
